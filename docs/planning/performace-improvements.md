...
## 2. Direct swap-chain write (eliminate output buffer copy) — TRIED, NOT WORTH IT

**Status (2026-05-02):** Implemented on branch `perf-impr-eliminate-output-buffer-copy`. Not merged — improvement
was unnoticeable in practice. Branch kept for reference.

**Implementation:** Added `dispatch_frame_to_surface` method; `shader_viewer.wgsl` writes
to `texture_storage_2d<bgra8unorm, write>` (group 1) instead of a storage buffer.
Backend-gated: Metal (TBDR) → buffer-copy fallback, Vulkan/DX12 (IMR) → direct write.

### What was learned

**WGSL component mapping:** `textureStore` vec4 is always `(R, G, B, A)` regardless
of the format name. `bgra8unorm` describes the *memory byte order*; the GPU swizzles
internally. Writing `vec4(fb, fg, fr, 1.0)` thinking "x=B" is wrong — correct is
`vec4(fr, fg, fb, 1.0)`.

**TBDR regression (M4 Max):** Adding `STORAGE_BINDING` to the surface texture forces
Metal to allocate it in a non-tiled layout, disabling on-chip tile memory for the
subsequent HUD render pass. Result: **12 fps → 9 fps** (-25%). The buffer→texture
copy path keeps the surface in framebuffer-optimal tiled layout; the HUD render pass
benefits from TBDR tile memory. The planning doc's "minor" estimate was wrong in the
*wrong direction* — the buffer copy is not just fast, it's *necessary* for TBDR
correctness.

**GTX 1650 (IMR, Vulkan):** No measurable improvement observed. The GPU DMA copy
(`copy_buffer_to_texture`) is fast enough at 192 GB/s that eliminating it saves
< 1 frame of overhead at these resolutions. The 2–5 ms estimate in the original doc
was optimistic.

**Why two WGSL files are unavoidable:** WGSL has no preprocessor. The buffer-path
shader (`shader_texture.wgsl`, used by `render_frame`/benchmarks) and the storage-
texture-path shader (`shader_viewer.wgsl`) differ only in 3 lines but cannot share
source without runtime string construction. This duplication cost alone was enough
reason not to merge.

### Verdict

The buffer→texture copy is not the bottleneck on either architecture tested. Do not
