...
// output — group 1, binding 0: swap-chain surface texture (bgra8unorm, write-only)
// textureStore vec4 component order is always x=R, y=G, z=B, w=A; GPU swizzles to bgra in memory
@group(1) @binding(0) var output: texture_storage_2d<bgra8unorm, write>;
// normals map
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if hit {
        ...
        // textureStore vec4 is always (R, G, B, A) — bgra8unorm describes memory layout, not shader order
        textureStore(output, vec2<i32>(gid.xy), vec4<f32>(fr / 255.0, fg / 255.0, fb / 255.0, 1.0));
    } else {
        // sky blue RGB(135, 206, 235)
        textureStore(output, vec2<i32>(gid.xy), vec4<f32>(135.0 / 255.0, 206.0 / 255.0, 235.0 / 255.0, 1.0));
    }
}
