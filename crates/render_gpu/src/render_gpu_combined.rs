...
pub fn render_gpu_combined(
    ...
    pollster::block_on(async {
        ...
        let nx_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("nx"), size: nm_size, mapped_at_creation: false,
            usage: wgpu::BufferUsages::STORAGE,
        });
        let ny_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("ny"), size: nm_size, mapped_at_creation: false,
            usage: wgpu::BufferUsages::STORAGE,
        });
        let nz_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("nz"), size: nm_size, mapped_at_creation: false,
            usage: wgpu::BufferUsages::STORAGE,
        });
        ...
        let normals_ub = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let normals_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("normals_bg"), layout: &normals_bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: normals_ub.as_entire_binding() },
                ...
                wgpu::BindGroupEntry { binding: 3, resource: nx_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 4, resource: ny_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 5, resource: nz_buf.as_entire_binding() },
            ],
        });
        ...
        let normals_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("normals_pl"), bind_group_layouts: &[Some(&normals_bgl)], immediate_size: 0,
        });
        let normals_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("normals_pipeline"), layout: Some(&normals_pipeline_layout),
            module: &normals_shader, entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(), cache: None,
        });

        // ── render pipeline ───────────────────────────────────────────────────
        let cam = CameraUniforms::new(origin, look_at, fov_deg, aspect, hm, sun_dir, width, height, step_m, t_max);
        ...
        let output_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("output"), size: (width * height * 4) as u64, mapped_at_creation: false,
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
        });
        let readback_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("readback"), size: (width * height * 4) as u64, mapped_at_creation: false,
        ...
        let render_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry { binding: 4, resource: nx_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 5, resource: ny_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 6, resource: nz_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 7, resource: shadow_buf.as_entire_binding() },
            ],
        });
        ...
        let render_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("render_pl"), bind_group_layouts: &[Some(&render_bgl)], immediate_size: 0,
        });
        let render_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("render_pipeline"), layout: Some(&render_pipeline_layout),
            module: &render_shader, entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(), cache: None,
        });
...

        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("normals_pass"), timestamp_writes: None,
            });
        ...
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("render_pass"), timestamp_writes: None,
            });
        ...
        encoder.copy_buffer_to_buffer(&output_buf, 0, &readback_buf, 0, (width * height * 4) as u64);

        queue.submit([encoder.finish()]);

        // ── readback image only ───────────────────────────────────────────────
        let (sender, receiver) = std::sync::mpsc::channel();
        readback_buf.slice(..).map_async(wgpu::MapMode::Read, move |r| sender.send(r).unwrap());
        device.poll(wgpu::PollType::Wait { submission_index: None, timeout: None }).unwrap();
        receiver.recv().unwrap().unwrap();

        let result = { let d = readback_buf.slice(..).get_mapped_range(); d.to_vec() };
        readback_buf.unmap();
        result
    })
}
