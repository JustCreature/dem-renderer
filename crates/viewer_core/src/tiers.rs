...
use dem_io::{Heightmap, crop, stitch_windows, stitch_windows_geographic};
...
pub struct TierRadii {
    pub base_radius_m: f64,
    pub base_drift_m: f64,
    pub close_radius_m: f64,
    pub close_drift_m: f64,
    pub fine_radius_m: f64,
    pub fine_drift_m: f64,
}
...
pub fn tier_radii(class: VramClass) -> TierRadii {
    match class {
...
pub fn cap_to_gpu_limit(hm: Heightmap, centre_e: f64, centre_n: f64) -> Heightmap {
...
pub struct StreamingTier {
    pub tx: mpsc::SyncSender<(f64, f64)>,
...
impl StreamingTier {
    ...
    pub fn needs_reload(&self, e: f64, n: f64) -> bool {
    ...
    pub fn try_trigger(&mut self, e: f64, n: f64) -> bool {
    ...
    pub fn try_recv(&mut self) -> Option<TierData> {
    ...
    pub fn update_threshold(&mut self, new_threshold_m: f64) {
        self.drift_threshold_m = new_threshold_m;
    }
}

/// Find the finest IFD level where scale ≥ `min_scale_m` and window fits in `max_px`.
pub fn select_ifd(scales: &[f64], min_scale_m: f64, radius_m: f64, max_px: u32) -> usize {
...
pub struct BevBaseState {
    pub base: StreamingTier,        // wide window, low resolution (IFD-2/1)
    pub close: StreamingTier,       // close window, 5 m/px (IFD-0)
    pub fine: Option<StreamingTier>, // fine window, 1 m/px (1m tile IFD-0); None if no 1m tiles available
}
...
impl BevBaseState {
    ...
    /// `tile_source` reads each window (filesystem natively, bytes on wasm) and `spawner`
    /// runs the three worker loops off the main thread.
    ...
    pub fn new(
        ...
        let ts_base = Arc::clone(tile_source);
            ...
            while let Ok((lat, lon)) = base_worker_rx.recv() {
                ...
                let windows: Vec<_> = overlapping
                    ...
                    .filter_map(|&i| {
                        ...
                        ts_base.read_window(&e.path, (cx, cy), radius, e.ifd).ok()
                    })
                    .collect();
                if windows.is_empty() {
                    continue;
        ...
        let ts_close = Arc::clone(tile_source);
        spawner.spawn(Box::new(move || {
            while let Ok((lat, lon)) = hm5m_worker_rx.recv() {
                ...
                let Ok(hm5m_raw) =
                    ts_close.read_window(&entry.path, (cx, cy), close_radius_m, entry.ifd)
                else {
                    continue;
                };
                ...
                let gpu_normals_rg16 = render_gpu::pack_normals_rg16_bytes(&normals.nx, &normals.ny);
        ...
        if let Some(&ci) = overlapping_close.first() {
            ...
            if let Ok((cx, cy)) = dem_io::crs::from_wgs84(cam_lat, cam_lon, &entry.crs_proj4)
                && let Ok(hm5m_init) =
                    tile_source.read_window(&entry.path, (cx, cy), close_radius_m, entry.ifd)
            {
        ...
        let fine = if fine_index.is_empty() || fine_radius_m <= 0.0 {
            ...
            let ts_fine = Arc::clone(tile_source);
            spawner.spawn(Box::new(move || {
                while let Ok((lat, lon)) = hm1m_worker_rx.recv() {
                    ...
                    let windows: Vec<_> = overlapping
                        ...
                        .filter_map(|&i| {
                            ...
                            ts_fine.read_window(&e.path, (et, nt), fine_radius_m, e.ifd).ok()
                        })
                        .collect();
                    if windows.is_empty() {
                        continue;
...
/// used for origin_x/y.
pub fn cross_crs_world_origin_and_extent(
    hm: &Heightmap,
    base_hm: &Heightmap,
) -> (f32, f32, f32, f32, f32) {
...
pub fn cross_crs_world_origin(hm: &Heightmap, base_hm: &Heightmap) -> (f32, f32) {
    if hm.crs_proj4 == base_hm.crs_proj4 {
        return (
