...
fn get_value_from_neighbours(data: &[i16], current_row: usize, current_col: usize, rows: usize, cols: usize, nodata: i16) -> i16 {
...
fn build_grayscale_png(heightmap: &Heightmap, cols: usize, rows: usize) {
    ...
    let pixels: Vec<u8> = heightmap.data.iter().map(|&e| {
        let e = e as f32;
        ((e - min) / (max - min) * 255.0) as u8
    }).collect();

    image::GrayImage::from_raw(cols as u32, rows as u32, pixels).unwrap().save("heightmap.png").unwrap();
}
...
pub fn parse_bil(bil_path: &Path) -> Result<Heightmap, DemError> {
    ...
    if bil_bytes.len() != expected_size {
        return Err(format!("size mismatch; expected: {}; got: {};", expected_size, bil_bytes.len()).into());
    }

    // convert to Vec<i16>
    let mut bil_data: Vec<i16> = bil_bytes.chunks_exact(2).map(|chunk| {
    ...
    let min = bil_data.iter().filter(|&&v| v != hdr_map.nodata).copied().min();
    let max = bil_data.iter().filter(|&&v| v != hdr_map.nodata).copied().max();
